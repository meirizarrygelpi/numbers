...
// A Rat is an dual-Cockle quaternion with big.Rat components.
type Rat struct {
	l, r cockle.Rat
}
...
// NewRat returns a pointer to the Rat value a+bi+ct+du+eΓ+fiΓ+gtΓ+huΓ.
...
func (z *Rat) Quad() *nplex.Rat {
	q := new(cockle.Rat)
	q.Mul(&z.l, q.Conj(&z.r))
	q.Scale(q, big.NewRat(2, 1))
	return nplex.NewRat(z.l.Quad(), q.Real())
}
