...
// A Float64 is an dual-Cockle quaternion with float64 components.
type Float64 struct {
	l, r cockle.Float64
}
...
//
// If z corresponds to a+bi+ct+du+eΓ+fiΓ+gtΓ+huΓ, then the string is
// "(a+bi+ct+du+eΓ+fiΓ+gtΓ+huΓ)", similar to complex128 values.
...
// SetPair sets z equal to an dual-Cockle quaternion made with a given pair, and
// then it returns z.
...
// NewFloat64 returns a pointer to the Float64 value a+bi+ct+du+eΓ+fiΓ+gtΓ+huΓ.
...
func (z *Float64) Mul(x, y *Float64) *Float64 {
	a, b, temp := new(cockle.Float64), new(cockle.Float64), new(cockle.Float64)
	a.Mul(&x.l, &y.l)
	b.Add(
		b.Mul(&x.l, &y.r),
		temp.Mul(&x.r, &y.l),
...
// Quad returns the quadrance of z. If z = a+bi+ct+du+eΓ+fiΓ+gtΓ+huΓ, then the
// quadrance is
// 		(a² + b² - c² - d²) + 2(ae + bf - cg - dh)Γ
// This is a nilplex number.
func (z *Float64) Quad() *nplex.Float64 {
	q := new(cockle.Float64)
	q.Mul(&z.l, q.Conj(&z.r))
	q.Dilate(q, 2)
	return nplex.NewFloat64(z.l.Quad(), q.Real())
}

// Norm returns the norm of z. If z = a+bi+ct+du+eΓ+fiΓ+gtΓ+huΓ, then the
// norm is
// 		(a² + b² - c² - d²)²
...
// 		Inv(y*c + d) * (y*a + b)
// Then it returns z.
func (z *Float64) MobiusL(y, a, b, c, d *Float64) *Float64 {
	z.Mul(y, a)
	z.Add(z, b)
	temp := new(Float64)
	temp.Mul(y, c)
...
// 		(a*y + b) * Inv(c*y + d)
// Then it returns z.
func (z *Float64) MobiusR(y, a, b, c, d *Float64) *Float64 {
	z.Mul(a, y)
	z.Add(z, b)
	temp := new(Float64)
	temp.Mul(c, y)
