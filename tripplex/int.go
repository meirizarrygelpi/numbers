...
// An Int is a tri-perplex number with big.Int components.
type Int struct {
	l, r bipplex.Int
}
...
// then it returns z.
func (z *Int) SetPair(a, b *bipplex.Int) *Int {
	z.l.Set(a)
	z.r.Set(b)
	return z
}

// NewInt returns a pointer to the Int value a+bs+cT+dsT+eU+fsU+gTU+hsTU.
...
func (z *Int) Mul(x, y *Int) *Int {
	a, b, temp := new(bipplex.Int), new(bipplex.Int), new(bipplex.Int)
	a.Add(
		a.Mul(&x.l, &y.l),
		temp.Mul(&x.r, &y.r),
	)
...
func (z *Int) Quad() *bipplex.Int {
	q := new(bipplex.Int)
	return q.Sub(q.Mul(&z.l, &z.l), new(bipplex.Int).Mul(&z.r, &z.r))
}
