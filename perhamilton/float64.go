...
// A Float64 is an perplex-Hamilton quaternion with float64 components.
type Float64 struct {
...
//
// If z corresponds to a+bi+cj+dk+eS+fiS+gjS+hkS, then the string is
// "(a+bi+cj+dk+eS+fiS+gjS+hkS)", similar to complex128 values.
...
// SetPair sets z equal to an perplex-Hamilton quaternion made with a given pair, and
// then it returns z.
...
// NewFloat64 returns a pointer to the Float64 value a+bi+cj+dk+eS+fiS+gjS+hkS.
...
func (z *Float64) Quad() *pplex.Float64 {
	...
	q.Dilate(q, 2)
	return pplex.NewFloat64(z.l.Quad()+z.r.Quad(), q.Real())
}
