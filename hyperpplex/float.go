...
// A Float is a hyper-perplex number with big.Float components.
type Float struct {
	l, r dualpplex.Float
}
...
// then it returns z.
func (z *Float) SetPair(a, b *dualpplex.Float) *Float {
	z.l.Set(a)
	z.r.Set(b)
	return z
}

// NewFloat returns a pointer to the Float value a+bs+cΓ+dsΓ+eΛ+fsΛ+gΓΛ+hsΓΛ.
...
func (z *Float) Mul(x, y *Float) *Float {
	a, b, temp := new(dualpplex.Float), new(dualpplex.Float), new(dualpplex.Float)
	a.Mul(&x.l, &y.l)
	b.Add(
		b.Mul(&x.l, &y.r),
		temp.Mul(&x.r, &y.l),
...
func (z *Float) Quad() *dualpplex.Float {
	q := new(dualpplex.Float)
